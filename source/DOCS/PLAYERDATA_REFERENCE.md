...
| `sanityCounter` | number | `0` | 0 – N (no hard limit) | `sanity.lua` (increments when `sanity` reaches 0) | "no repeated room" rule, `Utilities.checkSanityAchievements()`, gates ghost reveal (`entities/enemies/ghost.lua`, via `Config.Sanity.dangerCounterThreshold`), gates player glitch effect (`entities/player/glitch.lua`, same threshold) | Yes |
