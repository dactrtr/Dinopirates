...
| CrewMember | `greetingScriptsTiny` | `{"crew==0:0CMIsTiny"}` | list | Same grammar, checked in `CrewMember:returnScript()` on tiny A-press; takes priority over the per-crewId dialog. |
